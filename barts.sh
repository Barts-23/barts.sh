...
function certandkey () {
	cp ~/iambarts/ca.crt /etc/openvpn/
	cp ~/iambarts/server.key /etc/openvpn/
	cp ~/iambarts/server.req /etc/openvpn/
	cp ~/iambarts/server.crt /etc/openvpn/
	cp ~/iambarts/dh.pem /etc/openvpn/
}
...
cp /etc/openvpn/client-template.txt /home/panel/html/SunConfig.ovpn
echo 'http-proxy' $IP $PORTS >> /home/panel/html/SunConfig.ovpn
echo 'http-proxy-option CUSTOM-HEADER ""' >> /home/panel/html/SunConfig.ovpn
echo 'http-proxy-option CUSTOM-HEADER "POST https://viber.com HTTP/1.1"' >> /home/panel/html/SunConfig.ovpn
echo 'http-proxy-option CUSTOM-HEADER "X-Forwarded-For: viber.com"' >> /home/panel/html/SunConfig.ovpn
echo '<ca>' >> /home/panel/html/SunConfig.ovpn
cat /etc/openvpn/ca.crt >> /home/panel/html/SunConfig.ovpn
echo '</ca>' >> /home/panel/html/SunConfig.ovpn
}
...
function setall () {
rm /etc/issue.net
cat ~/iambarts/bann3r > /etc/issue.net
cat ~/iambarts/banner > /etc/motd
cp ~/iambarts/banner /etc/
	...
	case $PORT_PRIVO in
		...
		2)
			until [[ "$PORTS" =~ ^[0-9]+$ ]] && [ "$PORTS" -ge 1 ] && [ "$PORTS" -le 65535 ]; do
				read -rp "Custom port [1-65535]: " -e -i 8888 PORTS
			done
		;;
		3)
			# Generate random number within private ports range
...
cp ~/iambarts/nginx.conf /etc/nginx/nginx.conf
cp ~/iambarts/ocs.conf /etc/nginx/conf.d/vps.conf
...
echo 'Or http://'"$IP"':88/SunConfig.ovpn'
...
echo "======================================================="
echo "======================================================="
echo "For donations, Im accepting paymaya transactions and Prepaid loads: 09560753202"
history -c
rm -Rf ~/iambarts/
exit 0
